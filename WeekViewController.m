...
- (void)viewDidLoad
{
    ...
    if (isiPad) {
        ...
        self.dayPlannerView.dayHeaderHeight = 65;
    }
    else{
        //NSLog(@"---------------- iPhone ------------------");
        self.dayPlannerView.dateFormat = @"eee\nd \nMMM";
...
- (void)dayPlannerView:(MGCDayPlannerView*)view didSelectDayCellAtDate:(NSDate*)date{
    if ([self.delegate respondsToSelector:@selector(dayViewController:didSelectDayCellAtDate:)]) {
        [self.delegate dayViewController:self didSelectDayCellAtDate:date];
    }
}
...
- (void)dayPlannerView:(MGCDayPlannerView *)view didScrolledWithView:(CGPoint)_point{
    if ([self.delegate respondsToSelector:@selector(dayViewController:didScrolledFromMGCDayView:)]){
        [self.delegate dayViewController:self didScrolledFromMGCDayView:_point];
    }
}
...
- (BOOL)dayPlannerView:(MGCDayPlannerView*)view canMoveEventOfType:(MGCEventType)type atIndex:(NSUInteger)index date:(NSDate*)date toType:(MGCEventType)targetType date:(NSDate*)targetDate
{
    ...
    return YES;//(comps.weekday != 1 && comps.weekday != 7);
}
...
- (void)moveToDate:(NSDate*)date animated:(BOOL)animated
{
    if (!self.dayPlannerView.dateRange || [self.dayPlannerView.dateRange containsDate:date]) {
        [self.dayPlannerView scrollToDate:date options:MGCDayPlannerScrollDateTime animated:animated fromTap:NO];
    }
}
...
- (NSDate*)getCurrentDate{
    return [self.dayPlannerView getCurrentDate];
}
@end
